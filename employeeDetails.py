...
class employeeClass:
    def __init__(self, root):
        ...
        self.root.title("Employee Registration Form || TBS-Taxi Booking System || Developed By Sailesh Rokaya")
        ...
        #===========ProjectIcon=============
        photo = PhotoImage(file = "images/employee.png")
    ...
    #================Content=======================================
        ...
        #================TextFields-Row-1====================================
        txt_empId=Entry(self.root, textvariable=self.var_reg_id, stat="disable", font=("goudy old style",15),bg="lightyellow").place(x=150,y=150, width=180)
        ...
        self.RegistrationTable=ttk.Treeview(emp_frame,columns=("reg_id","name","address","gender","email","telephone","password","userType"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.RegistrationTable.xview)
        scrolly.config(command=self.RegistrationTable.yview)
        
        self.RegistrationTable.heading("reg_id",text="EMP ID")
        ...
        self.RegistrationTable.column("reg_id",width=50)
        ...
        try:
            if self.var_name.get() == "" or self.var_email.get()=="" or self.var_password.get()=="":
                ...
                if row != None:
                    ...
                    databaseConnection.cur.execute("Insert into registration (name,address,gender,email,telephone,password,userType) values (?,?,?,?,?,?,'Employee')", (
                    ...
                    messagebox.showinfo(
                        "Sucess", "Employee Added Sucessfully", parent=self.root)
                    self.clear()
                    self.show()
        ...
        try:
            if self.var_reg_id.get() == "":
                ...
                if row == None:
                    ...
                    databaseConnection.cur.execute("Update registration set name=?,address=?,gender=?,email=?,telephone=?,password=?,userType='Employee' where reg_id=?", (
                    ...
                    databaseConnection.con.commit()
                    messagebox.showinfo(
                        "Sucess", "Employee Updated Sucessfully", parent=self.root)
                    self.clear()
                    self.show()
        except Exception as ex:
            messagebox.showerror(
            ...
            if self.var_reg_id.get() == "":
                ...
                if row == None:
                    ...
                    if op == True:
                        ...
                        databaseConnection.con.commit()
                        messagebox.showinfo(
                            "Delete", "Empoyee Delete Successfully", parent=self.root)
                        self.clear()
                        self.show()
