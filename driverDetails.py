...
#======================================================================GUI_Designing======================================================================================
class driverDetailsClass:
    def __init__(self, root):
        self.root=root
        self.root.geometry("1100x500+300+130")
        self.root.title("Driver Details || TBS-Taxi Booking System || Developed By Sailesh Rokaya")
        ...
        self.var_licence=StringVar()
             
            
        #===========ProjectIcon=============
        photo = PhotoImage(file = "images/driver.png")
        self.root.iconphoto(False, photo)

        #===============SearchFrame=========================
        SearchFrame=LabelFrame(self.root, text="Search Driver", font=("goudy old style",12,"bold"), bd=2, relief=RIDGE, bg="white")
        ...
        #================titel====================================
        title=Label(self.root, text="Driver Details", font=("goudy old style",15),bg="#0f4d7d", fg="white").place(x=50,y=100, width=1000)

    #================Content=======================================
        #================Labels-Row-1====================================
        lbl_drivId=Label(self.root, text="Cust. ID", font=("goudy old style",15),bg="white").place(x=50,y=150)
        ...
        #================TextFields-Row-1====================================
        txt_drivId=Entry(self.root, textvariable=self.var_reg_id, stat="disable", font=("goudy old style",15),bg="lightyellow").place(x=150,y=150, width=180)
        ...
        #================Labels-Row-3====================================
        lbl_licence=Label(self.root, text="Licence Details", font=("goudy old style",15),bg="white").place(x=500,y=230)
        
        #===============ComboBoxForUserType-Row-3==============================
        txt_licence=Entry(self.root, textvariable=self.var_licence, font=("goudy old style",15),bg="lightyellow").place(x=635,y=230, width=180)
        ...
        self.RegistrationTable=ttk.Treeview(emp_frame,columns=("reg_id","name","address","gender","email","telephone","password","userType","licence","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.RegistrationTable.xview)
        scrolly.config(command=self.RegistrationTable.yview)
        
        self.RegistrationTable.heading("reg_id",text="Driver ID")
        ...
        self.RegistrationTable.heading("licence",text="Licence")
        self.RegistrationTable.heading("status",text="Status")

        self.RegistrationTable["show"]="headings"
        self.RegistrationTable.column("reg_id",width=60)
        ...
        self.RegistrationTable.column("licence",width=100)
        self.RegistrationTable.column("status",width=100)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "" or self.var_email.get()=="" or self.var_password.get()=="":
                ...
                databaseConnection.cur.execute("Select * from registration where email=?",(self.var_email.get(),))
                row = databaseConnection.cur.fetchone()
                if row != None:
                    messagebox.showerror(
                        "Error", "This email-id already exits, Try different", parent=self.root)
                else:
                    databaseConnection.cur.execute("Insert into registration (name,address,gender,email,telephone,password,userType,licence,status) values (?,?,?,?,?,?,'Driver',?,'Available')", (
                        ...
                        self.var_licence.get()
                        
                    ))
                    databaseConnection.con.commit()
                    messagebox.showinfo(
                        "Sucess", "New Driver Added Sucessfully", parent=self.root)
                    self.clear()
                    self.show()
    ...
    def update(self):
        databaseConnection
        try:
            if self.var_reg_id.get() == "":
                messagebox.showerror(
                    "Error", "Driver ID Must be required", parent=self.root)
            else:
                databaseConnection.cur.execute("Select * from registration where reg_id=?",
                            (self.var_reg_id.get(),))
                row = databaseConnection.cur.fetchone()
                if row == None:
                    messagebox.showerror(
                        "Error", "Invalid Driver ID.", parent=self.root)
                else:
                    databaseConnection.cur.execute("Update registration set name=?,address=?,gender=?,email=?,telephone=?,password=?,userType='Driver',licence=?, status='Available' where reg_id=?", (
                    ...
                    databaseConnection.con.commit()
                    messagebox.showinfo(
                        "Sucess", "Driver Updated Sucessfully", parent=self.root)
                    self.clear()
                    self.show()
        except Exception as ex:
            messagebox.showerror(
        ...
        try:
            if self.var_reg_id.get() == "":
                ...
                if row == None:
                    ...
                    if op == True:
                        databaseConnection.cur.execute("delete from registration where reg_id=?",
                                    (self.var_reg_id.get(),))
                        databaseConnection.con.commit()
                        messagebox.showinfo(
                            "Delete", "Driver Delete Successfully", parent=self.root)
                        self.clear()
                        self.show()
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get() == "Select":
                ...
                databaseConnection.cur.execute("select * from registration where " + 
